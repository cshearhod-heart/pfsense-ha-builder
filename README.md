...
`dhcpd/<iface>/dnsserver` has exactly the same trap and gets the same treatment. In both the ISC and Kea code paths, `services.inc` emits `option domain-name-servers {$ifcfgip}` — the interface's own real IP — whenever `dnsserver` is empty and the DNS Resolver or Forwarder is enabled. After the swap that's the non-floating address again, so DHCP clients keep their gateway on failover but lose DNS. The overrides are only written for interfaces in "reuse" mode. An interface with an **already-adopted** CARP VIP, or one using the "VIP is a new address" strategy, never gets them — in the adopt case because its live config is deliberately left untouched, in the new-VIP case because the tool can't know what downstream expects. So for both, the tool checks what pfSense *would* hand out and says so: if the DHCP gateway or DNS server resolves to the interface's own real IP rather than the VIP — whether it's set that way explicitly or just left empty for `services.inc` to fall back on — the output panel flags it as a danger callout naming the interface and both addresses. It is never rewritten. On an adopted interface that would mean editing live primary config, and `dhcpd` is synced config anyway: fixing only the secondary would be undone by the primary's first push. Fix it on the primary after import and let config sync carry it across.

This catches an HA pair whose *original* build missed the override — the pair looks healthy, CARP fails over correctly, and every DHCP client on the segment still loses its gateway and DNS. A gateway or DNS server pointing at some *other* address is treated as a deliberate choice and stays silent; only the node's own non-floating address is flagged.

The override is only written where that fallback would actually fire: an interface with explicit DNS servers already set — internal AD domain controllers, say — or a firewall running neither Resolver nor Forwarder (system DNS is handed out instead) is left exactly as it is. The point is to fix a fallback, not to impose the firewall as the DNS server. The output panel names the two sets separately, so it never claims an override that didn't happen.
...
| `sample-config-explicit-dns.xml` | The same fresh build, but the LAN scope already has its own DHCP DNS servers — the case where the DNS override must *not* fire. |
| `sample-config-adopted-dhcp-gap.xml` | An already-HA'd primary whose original build forgot the DHCP gateway/DNS overrides — the pair looks fine but hands clients a non-floating address. Must be flagged, never modified. |
